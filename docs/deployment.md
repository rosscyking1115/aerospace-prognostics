...
Prediction responses include `predicted_rul_lower`, `predicted_rul_upper`, `interval_method`, and `interval_quantile_level` when the loaded artifact carries interval calibration. For the current HGB policy artifact, the interval method is `train_residual_absolute_quantile` at quantile level 0.90.

**`interval_quantile_level` is a quantile level, not a coverage probability.** The served band is the 0.90 absolute quantile of the model's residuals **on its own training rows**, so it is in-sample and optimistic: the model has already seen every engine that set the width. Nothing about it bounds the probability that the interval contains the truth on a new engine. The repository does have intervals with a distribution-free coverage guarantee — split conformal with unit-grouped calibration, `aerospace-prognostics cmapss-conformal`, measured in [writeup.md](writeup.md) §6 — and they are deliberately **not** what the serving artifact carries today. Wiring them in is a change to the deployed uncertainty policy and gets its own round; it is tracked as claims-ledger item D7.

The field was called `interval_confidence` until 2026-08-09. The value never changed — only the name, which asserted a guarantee the method does not provide. A name travels with the data into every consumer, which is why disclosing the caveat in the docs was not sufficient. `tests/test_interval_naming.py` fails if the word returns to the prediction contract, the API schema, the database schema, or the calibration payload. The app database schema moved to `aerospace-prognostics/app-db/v2` for the column rename: there is no migration, and an existing local database is refused on open with an error naming both versions. Recreate it with `app-init-database` and re-run the prediction runs.
...
`generate-sbom` reads the locked `uv.lock` environment and writes a CycloneDX-style JSON software bill of materials. The CI workflow runs this command after `pip-audit`, so every pushed commit proves that the dependency inventory remains generatable from the lockfile.

The audit step itself is **advisory on pull requests**, not a merge gate. An advisory published overnight against a transitive dependency is not a property of the change under review, and treating it as one meant `main` went red twice in three weeks with nothing in the repository changing — and a security bump had to ride on an unrelated feature branch to unblock a merge. The failing signal lives in `.github/workflows/security-audit.yml`, which runs the same audit daily and emails the owner when it fails. On a pull request a failed audit still shows in the checks list and raises a workflow annotation; it just does not block.
...
CI also runs `scripts/ci_release_evidence_smoke.py`. The script uses tiny fixture telemetry to build a candidate package and then exercises prediction, artifact inspection, validation, benchmark, SBOM, promotion-report, dashboard payload, dashboard HTML, release-bundle, provenance, and fleet priority-policy validation evidence end to end. This is a plumbing smoke test rather than production model evidence, but it protects the release-gate workflow from silently breaking.
...
CI writes a serving-image manifest before container smoke checks. The manifest records image ID, repo tags, selected OCI labels, Docker healthcheck settings, whether `torch` was present in the runtime image, and validation booleans for required labels, healthcheck presence, revision matching, and dependency-surface expectations. CI then builds `artifacts/release/cmapss_fd001_release_bundle.json`, which links the model promotion evidence, dashboard payload, static dashboard HTML, and exact serving-image manifest, and `artifacts/release/cmapss_fd001_provenance.json`, which records source and workflow provenance for that release bundle. The workflow uploads a `ci-fd001-release-evidence` artifact containing reviewable JSON, Markdown, dashboard HTML, priority-policy validation, SBOM, and container manifest evidence, while leaving raw telemetry and the generated joblib model binary out of the upload.
