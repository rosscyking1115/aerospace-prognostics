...
| Optimizer | Adam |
| Loss | MSE by default; optional `nasa_surrogate`, MSE+NASA blended losses, asymmetric late-weighted MSE losses, target-weighted MSE, and mini-batch monotonic penalties |
| Random seed | 42 |
...
The comparison command now accepts prediction CSVs in addition to result tables, summarizing each prediction file into RMSE and NASA score before ranking. That keeps calibrated candidates in the same leaderboard without hand-built intermediate result CSVs. The refreshed FD001 leaderboard ranks HGB first, then the monotonic asymmetric-loss plus NASA-shift combo, then the original asymmetric-loss plus NASA-shift combo, then raw-MSE NASA-shift calibration, then the uncalibrated asymmetric and raw-MSE Transformer runs:
