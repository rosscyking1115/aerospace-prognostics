...
def test_cmapss_prediction_diagnostics_outputs_csv_and_markdown(tmp_path) -> None:
    ...
    _write_predictions(
        predictions_csv,
        [
            _prediction("FD001", "cnn", 1, 100.0, 110.0, end_cycle=10),
            _prediction("FD001", "cnn", 1, 99.0, 112.0, end_cycle=11),
            _prediction("FD001", "tcn", 2, 80.0, 55.0, end_cycle=10),
        ],
    )
...
def test_build_cmapss_prediction_monotonicity_diagnostics_tracks_unit_steps(
    tmp_path,
) -> None:
    predictions_csv = tmp_path / "predictions.csv"
    _write_predictions(
        predictions_csv,
        [
            _prediction("FD001", "cnn", 1, 100.0, 80.0, end_cycle=10),
            _prediction("FD001", "cnn", 1, 99.0, 82.0, end_cycle=11),
            _prediction("FD001", "cnn", 1, 98.0, 79.0, end_cycle=12),
            _prediction("FD001", "cnn", 2, 90.0, 90.0, end_cycle=5),
            _prediction("FD001", "cnn", 2, 89.0, 88.0, end_cycle=6),
            _prediction("FD001", "tcn", 3, 80.0, 75.0, end_cycle=6),
        ],
    )

    rows = build_cmapss_prediction_monotonicity_diagnostics(predictions_csv)
    ...
    cnn_row = rows[0]
    assert cnn_row.unit_count == 2
    assert cnn_row.transition_count == 3
    assert cnn_row.violation_count == 1
    assert cnn_row.violation_rate == 1 / 3
    assert cnn_row.mean_step_change == -1.0
    assert cnn_row.mean_violation_magnitude == 2.0
    assert cnn_row.max_violation_magnitude == 2.0
    tcn_row = rows[1]
    assert tcn_row.unit_count == 1
...
def _prediction(
    ...
    *,
    ...
    return {
        ...
        "end_cycle": end_cycle if end_cycle is not None else unit_number,
