...
def test_export_cmapss_sequence_splits_writes_npz_and_metadata(tmp_path) -> None:
    ...
    assert validation_selection_npz["windows"].shape == (1, 2, 24)
    assert validation_selection_npz["targets"].tolist() == [1.0]
    assert metadata["subset"] == "FD001"
    assert metadata["window_size"] == 2
    assert metadata["validation_selection_windows"] == 1
    assert metadata["validation_selection_npz_path"].endswith(
...
def test_export_cmapss_sequence_splits_writes_rolling_validation_selection(
    ...
    assert result.validation_windows == 1
    assert result.validation_selection_windows == 2
    assert validation_npz["targets"].tolist() == [1.0]
    assert validation_selection_npz["targets"].tolist() == [2.0, 1.0]
