...
| **D6** | **FD002 misses its nominal 99% coverage, and the cause is structural rather than statistical.** Splitting the measurement by whether the truth lies inside the model's expressible range: within the 125-cycle training cap coverage is `0.985149` over 202 engines; above the cap it is `0.631579` over 57 engines, 21 of them uncovered. FD002 puts 22% of its test fleet outside the range the predictor was trained to emit, and a symmetric interval centred on a systematically capped prediction cannot reach a truth beyond the cap however wide it is. | U4's coverage figure stands exactly as reported, and should not be read as a property of conformal prediction in general. It is evidence that distribution-free calibration cannot repair a systematically biased centre. | Fixing it means changing the point predictor — raising or removing the RUL cap — which moves every published FD002 number and needs its own evaluation. The two cheap alternatives were both rejected: widening the interval buys coverage with meaninglessness, and dropping above-cap engines from the report is choosing the population after seeing the result. |
