...
does not. A reader who conflates them will over-read the deployed system.
...
| # | Claim (with scope) | Produced by | Allowed to claim | Status |
|---|---|---|---|---|
| T1 | `534 passed`, full suite, measured 2026-08-09 on the conformal round (was `462 passed` on 2026-07-28) | `uv run pytest` | That the suite is green, across Python 3.11/3.12/3.13 in CI. The count is **not** pinned in a badge, because a hard-coded count rots silently | **Disclosed caveat — see D5** |
...
| **D7** | **The serving surface publishes a number called `interval_confidence` that is not a confidence level in the sense a reader will assume.** It is an in-sample train-residual quantile. Found by the repo-wide claims sweep run when the conformal work landed: the API field, the model card, `docs/deployment.md`, and the console's "Confidence" column all showed `0.90` with nothing saying what it was not. The number itself was never wrong; the surfaces around it invited one reading and supported another, and after conformal intervals entered the repository that gap became a real hazard. | No published figure changes. Every surface that shows the number now states that it is a target with no coverage guarantee, and points at the conformal work as the thing that does carry one. | Resolving it properly means either wiring unit-grouped conformal calibration into the packaged artifact, or renaming the API field — the first changes the deployed uncertainty policy and every served band, the second breaks the response contract and the SQLite schema. Both are their own round. Disclosing the gap is not a substitute for closing it, and this row stays open until one of them lands. |
