...
| # | Claim (with scope) | Produced by | Allowed to claim | Status |
|---|---|---|---|---|
| T1 | `462 passed`, full suite, measured 2026-07-28 | `uv run pytest` | That the suite is green, across Python 3.11/3.12/3.13 in CI. The count is **not** pinned in a badge, because a hard-coded count rots silently | **Disclosed caveat — see D5** |
...
| **D5** | **One known flake, unreproduced.** `test_seed_quickstart_workspace_persists_model_and_evidence` failed once on 2026-07-27 in a full-suite run whose preceding commit changed only markdown. It has passed on every attempt since — 20 consecutive runs across two branches, 10 of them targeting that file alone. Ruled out: test-order dependence (no randomising plugin is installed, so collection order is deterministic), shared state (per-test `tmp_path`), time dependence in the idempotency key (the conflict key is content-addressed), and parallelism (single-process). What was never established is which assertion failed — the traceback was not captured. | T1's "the suite is green" is true of every observed run but is **not** a claim that the suite is deterministic. A reader should treat one intermittent failure per few hundred runs as possible. | The cause is not known, and guessing at a fix would be worse than disclosing. Instead the mechanism is pinned by `test_a_missing_evidence_file_is_silently_skipped_not_reported`, which demonstrates that a missing evidence file yields a silent short count, and the flaky test now prints the evidence inputs on failure so the next occurrence diagnoses itself. Deliberately not a retry or an `xfail`: both would suppress the signal rather than resolve it. |
