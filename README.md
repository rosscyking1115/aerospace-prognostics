...
- `aerospace-prognostics phase2-cmapss` run manifests with artifact checksums, runtime provenance, Git state, verifier/audit reports, and optional NASA-surrogate, asymmetric, blended, target-weighted, and monotonic training losses for Track A experiment bundles.
- Phase 2 C-MAPSS deep prediction diagnostics for official-test and validation-selection per-window actual RUL, predicted RUL, signed error, absolute error, early/late error split, aggregate error summaries, RUL-bin calibration views, monotonicity checks, high-error unit reports, and validation-fitted calibration checks.
