...
- Streamlit local operations console with fleet triage, persisted fleet asset registry, spacecraft anomaly event upload with validation preview, priority-policy validation downloads, model registry, release-gate report cards, operational interval availability diagnostics, observed-outcome coverage/MAE diagnostics, artifact evidence, API-backed or local batch prediction, SQLite-backed prediction history, outcome-template downloads, read-only-safe prediction-run evidence and model-review bundle downloads, file-based evidence export, read-only demo mode, operator decisions, audit events, and API readiness status.
...
- `aerospace-prognostics app-export-priority-policy` CLI for exporting JSON and Markdown validation evidence for cross-domain fleet priority rules, with an optional failing release-gate mode.
