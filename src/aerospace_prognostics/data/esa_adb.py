...
Evaluator-contract logic (label joins, event grouping, metric-input
validation, event-wise scoring) lives in the ``telemeval`` library; this
module keeps only the dataset plumbing that is specific to this project
(source manifest, archive verification).
"""
...
from telemeval.contract import LABEL_COLUMNS as ESA_ADB_LABEL_COLUMNS
from telemeval.contract import PREDICTION_COLUMNS as ESA_ADB_PREDICTION_COLUMNS
from telemeval.formats.esa_adb import ESA_ADB_ANOMALY_TYPE_COLUMNS

from aerospace_prognostics.artifact_io import write_json_payload

__all__ = [
    "ESA_ADB_ANOMALY_TYPE_COLUMNS",
    "ESA_ADB_LABEL_COLUMNS",
